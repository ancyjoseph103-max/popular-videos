...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Standard direct HTTPS fallback videos working seamlessly across all browsers & devices
  ...
  function createVideoCard(video, index) {
    ...
    // Robust video URL handling: Check if it's a direct HTTPS link vs local device reference
    const rawVideoUrl = (video.videoUrl || '').trim();
    const isLocalIndexedDb = rawVideoUrl.startsWith('indexeddb://');
    const isBlobUrl = rawVideoUrl.startsWith('blob:');

    // Only assign initial src directly if it's a standard web URL (http:// or https://)
    let initialVideoSrc = '';
    if (!isLocalIndexedDb && !isBlobUrl && rawVideoUrl) {
      initialVideoSrc = rawVideoUrl;
      if (!initialVideoSrc.includes('#')) {
        initialVideoSrc += '#t=0.001';
      }
    }
    ...
    card.innerHTML = `
      ...
      <div class="video-wrapper">
        <video 
          class="video-player" 
          src="${escapeHtml(initialVideoSrc)}"
          ${posterAttr}
          playsinline 
          loop
          preload="auto"
    ...
    // Video player element resolution & remote client fallback
    const videoPlayerEl = card.querySelector('.video-player');
    const defaultFallbackUrl = FALLBACK_VIDEOS[index % FALLBACK_VIDEOS.length];

    if (isLocalIndexedDb && window.localVideoStorage) {
      window.localVideoStorage.resolveVideoUrl(rawVideoUrl).then((resolved) => {
        if (resolved && videoPlayerEl) {
          videoPlayerEl.src = resolved;
          videoPlayerEl.load();
        } else {
          // Stored on another device: cleanly fallback to direct HTTPS stream so screen is never black
          console.warn(`[Video Feed] Local file (${rawVideoUrl}) not found on this device. Applying direct HTTPS fallback.`);
          if (videoPlayerEl) {
            videoPlayerEl.src = defaultFallbackUrl + '#t=0.001';
            videoPlayerEl.load();
          }
        }
      }).catch((err) => {
        console.warn(`[Video Feed] Error resolving IndexedDB video:`, err);
        if (videoPlayerEl) {
          videoPlayerEl.src = defaultFallbackUrl + '#t=0.001';
          videoPlayerEl.load();
        }
      });
    } else if (isLocalIndexedDb || isBlobUrl || !initialVideoSrc) {
      // Local reference on a remote browser without local storage support
      console.warn(`[Video Feed] Video references local storage unavailable on this client. Applying direct HTTPS fallback.`);
      if (videoPlayerEl) {
        videoPlayerEl.src = defaultFallbackUrl + '#t=0.001';
        videoPlayerEl.load();
      }
    ...
    // Graceful error recovery: if video fails to load for any reason (CORS, 404, unsupported codec)
    if (videoPlayerEl) {
      videoPlayerEl.addEventListener('error', () => {
        console.warn(`[Video Feed] Video load error for ${video.title || 'video'}. Recovering with HTTPS stream...`);
        if (videoPlayerEl.src !== defaultFallbackUrl && videoPlayerEl.src !== defaultFallbackUrl + '#t=0.001') {
          videoPlayerEl.src = defaultFallbackUrl + '#t=0.001';
          videoPlayerEl.load();
        }
      }, { once: true });
  ...
  function attachCardEventListeners(card, video) {
    ...
    playRing.addEventListener('click', (e) => {
      ...
      function unlockVideo() {
        ...
        if (!videoElem.src || videoElem.src === window.location.href || videoElem.src.startsWith('indexeddb://') || videoElem.error) {
          videoElem.src = defaultFallbackUrl;
          videoElem.load();
        }
