...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 1. Initialize Real-Time Top Banner Ad (Strictly Conditional)
  if (window.dataService) {
    window.dataService.subscribeBanner((bannerData) => {
      const bannerHtml = (bannerData && bannerData.html) ? bannerData.html.trim() : '';
      // Only render if ad content is explicitly configured; otherwise keep area completely clean
      if (bannerHtml && !bannerHtml.includes("Special Offer")) {
        topBannerInner.innerHTML = bannerHtml;
