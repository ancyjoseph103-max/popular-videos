...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (window.dataService) {
    ...
    // Real-time Banner Subscription (Strictly conditional)
    window.dataService.subscribeBanner((bannerData) => {
      ...
      if (bannerHtml && !bannerHtml.includes("Special Offer")) {
        bannerHtmlInput.value = bannerHtml;
        bannerPreviewContent.innerHTML = bannerHtml;
        statBannerStatus.textContent = "Active";
      } else {
        bannerHtmlInput.value = "";
        bannerPreviewContent.innerHTML = '<span style="color: var(--text-muted); font-size: 13px; font-style: italic;">No banner configured. Top area remains completely clean.</span>';
        statBannerStatus.textContent = "Empty (Clean)";
      }
    });
  }
      ...
      card.innerHTML = `
        ...
        <div class="video-item-info">
          ...
          <div class="video-item-links">
            ${isLocal ? `
              <span class="link-chip" title="Direct Local Video File">
                <span>📹 Video:</span>
                <span>Local File (Stored)</span>
              </span>
            ` : `
              <a href="${safeVideoUrl}" target="_blank" class="link-chip" title="Direct Video MP4">
                <span>📹 Video:</span>
  ...
  addVideoForm.addEventListener('submit', async (e) => {
    ...
    if (!selectedFile && !directUrl) {
      showAdminToast("Please choose a video file OR enter a direct video URL!");
      return;
    }
    ...
    if (directUrl && (directUrl.includes('youtube.com') || directUrl.includes('youtu.be'))) {
      showAdminToast("YouTube watch links cannot be played directly in HTML5 video tags. Please provide a direct .mp4/.webm video link or choose a local file.");
      return;
    }
    ...
    try {
      ...
      // If a local video file is selected, store directly without external APIs
      if (selectedFile) {
      ...
      showAdminToast("Video published to feed successfully! 🎉");
  ...
  // 6. Handle Top Banner Form & Live Preview
  ...
  bannerForm.addEventListener('submit', async (e) => {
    e.preventDefault();
    const bannerHtml = bannerHtmlInput.value.trim();

    try {
      await window.dataService.updateBanner(bannerHtml);
      if (bannerHtml) {
        showAdminToast("Top Banner Ad updated successfully! 🚀");
      } else {
        showAdminToast("Top banner cleared. Top area is now completely clean! ✨");
      }
    } catch (err) {
      console.error("Banner update error:", err);
      showAdminToast("Failed to update banner: " + err.message);
    }
  });
  ...
  if (clearBannerBtn) {
    clearBannerBtn.addEventListener('click', async () => {
      ...
      bannerPreviewContent.innerHTML = '<span style="color: var(--text-muted); font-size: 13px; font-style: italic;">No banner configured. Top area remains completely clean.</span>';
      try {
        await window.dataService.updateBanner('');
        showAdminToast("Top banner removed. Top area is now clean! ✨");
      } catch (err) {
