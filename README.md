...
### 2. Streamlined Admin Dashboard & Ad Engine (`rifat.html`)
- **4 Essential Video Fields**:
...
- **Dynamic Ad Script Injection Engine**:
  * Evaluates `<script>` tags dynamically using `document.createElement('script')` in sequential order (guarantees Adsterra `atOptions` executes before `invoke.js`).
  * Centers iframes and banner containers responsively below the LIVE header without colliding with sound controls.
  * Document.write safety sandbox polyfill preventing legacy ad code from wiping page contents.
- **Global Popunder / On-Click Ad Support**:
  * Directly appends full-page popunder scripts (Adsterra, PropellerAds, Monetag) to `document.body`/`document.head` so clicks trigger popunders site-wide.
  * Real-time sync and status monitoring in Admin Dashboard.
